exports.handler = async (event) => {
  ...
  try {
    ...
    const amazonOrderId = body.amazonOrderId || `amazon-test-${Date.now()}`;
    ...
    const roastifyPayload = {
      externalSourceId: amazonOrderId,
      toAddress: {
        name: body.customerName || "Amazon Customer",
    ...
    const response = await fetch("https://api.roastify.app/v1/orders", {
      ...
      headers: {
        ...
        "Content-Type": "application/json",
        "Idempotency-Key": body.idempotencyKey || amazonOrderId
      },
      body: JSON.stringify(roastifyPayload)
    });
